...
function paintToCanvas() {
    ...
    return setInterval(() => {
        ...
        ctx.globalAlpha = 0.1; // "ghost blur" - places a transparency of 10% of the current image on top for 10 frames
...
function rgbSplit( pixels ) {
    for ( let i = 0; i < pixels.data.length; i+=4 ) {
        ...
        // pixels.data[i - 1500] = pixels.data[i + 0]; // red
...
function greenScreen( pixels ) {
    ...
    for ( i = 0; i < pixels.data.length; i += 4 ) {
        ...
        if ( red >= levels.rmin
            ...
            pixels.data[i + 3] = 0; // b/c every fourth element is the alpha value which makes things transparent
        }
    }
